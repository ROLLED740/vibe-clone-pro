...
import { startClipsCheckout } from "../actions/clips-billing";
import {
  PLANS,
  FAQ,
...
export default function ClipsPricingTable({ isSignedIn, currentPlanId }: PricingTableProps) {
  ...
  const handleCheckout = (plan: Plan) => {
    ...
    startTransition(async () => {
      try {
        const res = await startClipsCheckout(plan.id, interval);
        if (res?.url) {
...
function SocialProof() {
  ...
  return (
    <section className="mt-20 text-center">
      <h2 className="text-xl font-bold">Teams already using VibeClips</h2>
...
function PlanButton({
  ...
  if (plan.cta.kind === "signup") {
    return (
      <Link href={isSignedIn ? "/clips/studio" : "/sign-up"} className={`${base} ${style}`}>
  ...
  if (!isSignedIn) {
    return (
      <Link href="/sign-in?redirect_url=/clips/pricing" className={`${base} ${style}`}>
        {plan.ctaLabel}
      </Link>
    );
  }
