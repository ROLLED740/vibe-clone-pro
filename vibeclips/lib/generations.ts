...
import { getProvider, isAspectRatio, type MediaKind } from '@/lib/provider';
import { buildPrompt, getTemplate } from '@/lib/templates';
import { FREE_DAILY_PREVIEWS } from '@/lib/plans';
...
export async function ensureUserRow(userId: string, email?: string | null) {
  ...
  await database
    .insert(users)
    .values({ id: userId, email: email || `${userId}@users.noreply.vibeclips.app` })
    .onConflictDoNothing();
}
